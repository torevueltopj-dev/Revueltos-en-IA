...
const GEMINI_API_KEY = /* set as secret in Cloudflare */;
const DEEPSEK_KEY = /* set as secret in Cloudflare */;
...
// Buscar usando Gemini DeepResearch
async function handleSearch(request){
  try{
    ...
    // Filtrar solo dominios oficiales
    const items = (data.results||[]).filter(r=>/(\.org|\.gov|\.un\.org)/i.test(r.url)).map(r=>({
      title:r.title,url:r.url,domain:(new URL(r.url)).hostname,date:r.date,summary:r.snippet
    }));
    return json({ type:"search", items });
  }catch(e){ return json({error:e.message},500); }
}

// Funciones MUN usando Deepsek
